  ...
  if (pepperTweakerConfig.pluginEnabled) {
    ...
    if (pepperTweakerConfig.darkThemeEnabled) {
      ...
      css += `
        ...
        .border--color-greyBackground, .page-divider, .popover-item, .boxSec-divB, .boxSec--fromW3, .cept-comment-link, .border--color-borderGrey, .border--color-greyTint, .staticPageHtml table, .staticPageHtml td, .staticPageHtml th {
          border-color: ${lightBorderColor};
        }
    ...
    if (pepperTweakerConfig.pluginEnabled && ((location.pathname.length < 2) || location.pathname.match(/search|gor%C4%85ce|nowe|grupa|om%C3%B3wione|promocje|kupony[^\/]|dyskusji|profile/) || 1 == 1)) {
      ...
      if (dealsSection) {
        ...
        if (pepperTweakerConfig.improvements.listToGrid && !isGridLayout) {
          ...
          const sideContainerWidth = sideWidgetsWidth.reduce((acc, cur) => acc || (isNumeric(cur) && cur > 0), false) ? 234 : 178;
